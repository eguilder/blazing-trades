#!/usr/bin/env python3
"""Fetch an IBKR option chain and calculate max-pain levels.

The standard max-pain calculation minimizes expiration intrinsic value using
open interest.  ``premium_max_pain`` is also reported: it maximizes the
current premium (midpoint, or last-price fallback) that would expire
worthless, weighted by open interest.  Premium data is a market snapshot and
is not part of the standard max-pain definition.
...
def midpoint(bid: object, ask: object, last: object) -> float:
    """Return bid/ask midpoint, falling back to last trade or zero."""
    if valid_number(bid) and valid_number(ask) and float(bid) > 0 and float(ask) > 0:
        return (float(bid) + float(ask)) / 2
    if valid_number(last) and float(last) > 0:
        return float(last)
    return 0.0


def calculate_max_pain(rows: Iterable[OptionRow]) -> dict[str, object]:
    """Calculate OI payout max pain and premium-weighted worthless premium.
    ...
    worthless_premium_by_strike: dict[float, float] = {}
    for settlement in strikes:
        ...
        for row in rows:
            ...
            if (row.right == "C" and settlement < row.strike) or (
                row.right == "P" and settlement > row.strike
            ):
                worthless += row.premium * contracts
        payout_by_strike[settlement] = payout
        worthless_premium_by_strike[settlement] = worthless

    oi_level = min(payout_by_strike, key=lambda strike: (payout_by_strike[strike], strike))
    premium_level = max(
        worthless_premium_by_strike,
        key=lambda strike: (worthless_premium_by_strike[strike], -strike),
    )
    return {
        "max_pain_strike": oi_level,
        "max_pain_payout": payout_by_strike[oi_level],
        "premium_max_pain_strike": premium_level,
        "worthless_premium": worthless_premium_by_strike[premium_level],
        "payout_by_strike": payout_by_strike,
        "worthless_premium_by_strike": worthless_premium_by_strike,
    }
...
def fetch_rows(ib: IB, args: argparse.Namespace, expiration: str) -> tuple[list[OptionRow], float]:
    ...
    # Use streaming requests, matching the working Greeks service.  Snapshot
    # requests can complete before generic open-interest ticks are delivered.
    tickers = [ib.reqMktData(contract, "100,101", snapshot=False, regulatorySnapshot=False)
    ...
    for contract, ticker in zip(contracts, tickers):
        ...
        rows.append(OptionRow(
            ...
            premium=midpoint(ticker.bid, ticker.ask, ticker.last),
...
def write_csv(path: str, result: dict[str, object]) -> None:
    ...
    worthless = result["worthless_premium_by_strike"]
    with open(path, "w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(("settlement_strike", "oi_expiration_payout", "worthless_premium"))
        for strike in sorted(payout):
            writer.writerow((strike, payout[strike], worthless[strike]))
...
def write_chain_csv(path: str, rows: Iterable[OptionRow]) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(("strike", "right", "open_interest", "premium", "multiplier"))
        for row in sorted(rows, key=lambda item: (item.strike, item.right)):
            writer.writerow((row.strike, row.right, row.open_interest, row.premium, row.multiplier))
...
def main() -> int:
    ...
    try:
        ...
        print(f"Premium max pain:   {result['premium_max_pain_strike']:.2f} "
              f"(premium expiring worthless ${result['worthless_premium']:,.0f})")
        if args.output:
